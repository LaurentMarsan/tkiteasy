...
################################################################################
    ...
    def mkbox(self,x,y,w): #w = taille en unités de texte, pas pixels
        self.v = tk.StringVar()
        e = tk.Entry(self,textvariable=self.v,width=w)
        return ObjetGraphique(self.create_window(x,y,window=e),x,y,None)

    def cb1(self):
        print("yo",self.v)


    def button(self,txt,x,y):
        b = tk.Button(self,text=txt,command=self.cb1)
        return ObjetGraphique(self.create_window(x,y,window=b),x,y,None)
